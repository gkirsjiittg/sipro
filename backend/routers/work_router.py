...
async def list_tasks(scope: str = None, filter: str = None, type: str = None,
    ...
    Fase 40: pencarian judul, filter multi (tipe/status/prioritas), dan sort server-side.
    """
    ...
    lst.apply_in(base, "type", type)
    lst.apply_in(base, "priority", priority)
    lst.apply_search(base, q, ("title", "description", "outcome"))
    q_mongo = await _scope_query(user, scope, base)
    if division and (wh.is_owner_level(user) or wh.division_of(user) == division):
        q_mongo["division"] = division
    if assigned_to and scope in ("division", "all"):
        q_mongo["assigned_to"] = assigned_to
    ...
    all_rows = await db.tasks.find(q_mongo, {"_id": 0}).sort("due_date", 1).to_list(1000)
